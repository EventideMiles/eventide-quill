...
- **Linting: ESLint** with `eslint-plugin-obsidianmd` `^0.4.x` (configured in `eslint.config.mts`). The 0.4.x line is the "community scanners ruleset" — it mirrors what the Obsidian plugin reviewer flags, and downgrades every rule to `warn` by default (none of them fail `npm run lint` on their own). We override `obsidianmd/ui/sentence-case` back up to `error` so a sentence-case violation will fail the build; everything else (including `prefer-create-el`) stays at `warn`, so `npm run lint` only exits non-zero on actual errors or stylelint/jscpd failures. The 0.4.x line also adds `prefer-create-el`, `prefer-setting-definitions`, `no-deprecated-display`, `no-nodejs-modules` to the recommended config. `prefer-setting-definitions` is active — 2.2.0 adopted the declarative `getSettingDefinitions()` API (see "Coding rules" below). Additionally, `eslint-plugin-jsdoc` `^63.3.2` provides the **`jsdoc/require-jsdoc` docstring gate** (also `warn`-level, so `npm run lint` stays green) — see "Coding rules" below.
...
- **Duplication: jscpd v5** (Rust engine; dev-only, never bundled into `main.js`) — copy-paste detector gated in CI via `npm run lint:dup`. Configured in `.jscpd.json`: `minLines` 6, `minTokens` 70, `mode: weak` (skip comment tokens so JSDoc doesn't inflate the count), `threshold: 1.8` over duplicated **lines** (baseline 1.60% post-2.2.0 co-writer/main turn-extraction; **ratchet down** as duplication is extracted — the number should only ever decrease). The v5 Rust engine dropped v4's inline `jscpd:ignore-file` directive, so file-level exclusion lives in `.jscpd.json`, not at the code site.
- **Types: `obsidian`** type definitions (configured in `tsconfig.json`)
- **Dependency overrides** (`package.json` `overrides`) — keep `npm audit` at zero without breaking changes: `mocha`'s `serialize-javascript` → `^7.1.0` (RCE fix; CJS entry retained, mocha only uses it in parallel-worker mode); deprecated `glob` v8/v10 lines → `^13.0.6` under `mocha` / `@wdio/config` / `archiver-utils` (all three consumers use only the API-stable `sync`/`globSync` surface); `@wdio/utils`' `@puppeteer/browsers` → `^3.2.0` (drops the vulnerable `extract-zip` for `modern-tar`; all WDIO-used exports — `install`, `canDownload`, `detectBrowserPlatform`, `computeExecutablePath`, `Browser`, `ChromeReleaseChannel` — verified present in 3.x); `eslint`'s nested `minimatch` → `brace-expansion@^1.1.18` (in-range DoS fix). All findings were dev-only — nothing vulnerable ships in `main.js`. Drop each override once its parent declares the fixed range natively.
...
| `npm run build`              | **Three stages:** `sass` → `tsc -noEmit -skipLibCheck` (typecheck, no emit) → esbuild **production** build (`__DEV__ = false`, no sourcemaps, minified). This is the release artifact build — the Obsidian automated reviewer reconstructs `main.js` from source via `npm ci && npm run build`, so `npm run build` MUST produce a bundle byte-equivalent to the `main.js` attached to the GitHub release. Used by `release.yml` and `release.mjs`. NOTE: no `prettier --write` here (that's a source-mutation step, kept out of the reproducible build; use `lint:fix` or `build:dev` to format). |
| `npm run build:dev`          | **Four stages:** `sass` → `prettier --write` → `tsc -noEmit -skipLibCheck` → esbuild **dev** build (`__DEV__ = true`, sourcemaps inline, no minify). **Agents running the build pipeline locally should prefer `build:dev`** — it keeps the dev-only flags (`__DEV__`, inline sourcemaps) active during iteration and formats the source in one pass. Never ship a `build:dev` output as a release artifact (it won't match the release `main.js` and will trip the reviewer's reproducibility check). |
...
| `npm run test:e2e:live`      | Same as `test:e2e` but scoped to `test/specs/live/**` (via `E2E_LIVE=1`) — exercises the real local model. Auto-skips when LM Studio is unreachable at `http://localhost:1234/v1/models`. The live harness picks a dedicated embed model (id matching `/embed\|nomic\|bge\|e5/`) for the embed provider when one is loaded, so analysis/feedback live specs exercise the real embeddings path. Local-only; never in CI.                                                                                                                                                                                                                                                |
...
1. `npm run build` (sass + `tsc` + esbuild — production build that must match the released `main.js`). For local iteration with dev flags on, use `npm run build:dev` instead.
...
10. `npm run build` already exercises minification and `__DEV__` tree-shaking (it is the production/release build). Run `npm run build` (not `build:dev`) before tagging a release to confirm the bundle is byte-equivalent to what `release.yml` will attach.

CI (`.github/workflows/lint.yml`) runs `build` + `test` + `lint` + `lint:dup` + `lint:orphans` on every push and PR across Node 22/24 (Node 20 dropped after EOL — a dev-only transitive dep, `@electron/get` v5 via `obsidian-launcher`, requires Node ≥22.12). CI for releases (`.github/workflows/release.yml`) also runs `build` (same production command, byte-reproducible). The mocked E2E suite runs in its own job (`.github/workflows/e2e.yml`) on every PR — Ubuntu-latest, latest Obsidian, single instance — adds ~5min to the PR check (the ~100MB Obsidian bundle is cached via `actions/cache`). The live suite is local-only and never in CI.
...
| `mobile-responsive.e2e.ts`              | Mobile-emulation CSS: co-writer compact-width + hamburger overflow engage at 390px; dashboard/review no horizontal overflow (the feature specs skip on mobile, so this carries mobile layout coverage) |
| `live/lm-studio-smoke.e2e.ts` (live)    | Real LM Studio discuss round + follow-up; auto-skips if unreachable |
| `live/lore-consistency.e2e.ts` (live)   | Real LM Studio run of the 2.2.0 lore-consistency critical mode (line-referenced report); auto-skips if unreachable |
...
- **Production builds** (`npm run build`): `__DEV__ = false`
- **Dev builds** (`npm run build:dev`, `npm run dev`): `__DEV__ = true`
...
src/
  ...
  core/
    ...
    dashboard/            # Manuscript dashboard + lorebook
      ...
      writing-goals.ts (2.2.0 daily words-written ledger + streak + focus-session core with idle-timeout auto-stop; pure logic + sidecar persistence),
    ...
    memories/             # Cross-session AI-managed context (writer-editable markdown)
      memory-file.ts (pure-logic parser/serializer + block-ID minting/re-tokenize + index builder),
      memory-scope.ts (top-level-folder scope resolver + exclusion list + isMemoryFilePath raw-edit guard),
      memory-store.ts (vault-aware read/write + assignMissingIds-on-read + readActiveAndGlobal + removeMemoryEntry),
      memory-confirm.ts (ConfirmModal Promise wrapper — used by save_memory when memoriesAutoSave is off and by delete_memory always)
  ai/                  # Provider architecture, streaming, prompts, tools, vision
    ...
    memory-prompts.ts (buildMemoryMessage — auto-injected memory index for co-writer context + MEMORY_DISCIPLINE_CLAUSE telling the model when to save),
    ...
    tools/                # Tool-calling layer (see "Tool-calling architecture")
      ...
      save-memory.ts, recall-memory.ts, delete-memory.ts (memory CRUD tools — gated by memoriesEnabled, registered via registerMemoryTools on every parent-mode registry when on),
...
| Memories (default on)           | `save_memory`, `recall_memory`, `delete_memory`                                                                                                                                                                    | `memoriesEnabled` (independent gate — registered on every parent-mode registry when on; read-only subagent gets `recall_memory` only; off = no tools, no `MEMORY_DISCIPLINE_CLAUSE` in the prompt, no `buildMemoryMessage` index injection — feature vanishes entirely from the model's awareness)             |
...
### `'review-discuss'` mode (v2.2.0 — proactive editor chat)
...
Cross-session AI-managed context, stored as writer-editable markdown. When the model learns something durable (a voice preference, an intent behind a flagged choice, a worldbuilding fact not in the lorebook), it calls `save_memory` to persist it; future sessions see the memory via an auto-injected index. The writer can read, edit, and delete memories directly in the markdown file (Lorebook-sovereignty pattern) — the UI is a view, not a separate editor.

- **Scope**: top-level folder of the active manuscript = the memory pool. Files at vault root fall through to the global pool. The lorebook folder is NOT excluded — it gets its own pool (useful for lorebook-coach preferences). System folders (`.obsidian` via `Vault#configDir`, `.trash`, the memories folder itself) are excluded.
- **Storage**: markdown in `Memories/` (configurable). One `<scope-key>.memories.md` per top-level folder + `_global.memories.md`. Each memory is a `## section` with a trailing `^quill-mem-NNN` Obsidian block ID as the stable handle for delete/update. Writer-added sections without an ID get one minted on next read (re-tokenize pass).
- **Tools** (`src/ai/tools/{save,recall,delete}-memory.ts`): `save_memory` (append or update-by-heading, scope arg `'auto'|'manuscript'|'global'`); `recall_memory` (query + tags filter + `'all'` for union); `delete_memory` (by block ID, always confirms via modal). All three gated by `memoriesEnabled`.
- **Hybrid retrieval**: at session start, `buildMemoryMessage` (`src/ai/memory-prompts.ts`) reads the active + global pools, builds an index (heading + first sentence per entry, capped at `memoriesMaxIndexEntries`), and injects it as a system message. `memoriesFullInject` replaces previews with full bodies for powerful models. `recall_memory` fetches full bodies on demand.
- **Raw-edit guard**: `edit_note` / `insert_note` / `append_to_note` / `revise_edit` / `delete_paragraph` refuse to operate on any path under `memoriesFolder` (`isMemoryFilePath` check at the top of each tool's `execute`). The model can't bypass `save_memory` / `delete_memory` and clobber block IDs by editing the raw markdown.
- **Approval flow**: when `memoriesAutoSave` is on (default), `save_memory` writes directly with a Notice. When off, it shows a confirm modal. `delete_memory` ALWAYS shows a confirm modal regardless of toggle — silent destructive ops are too risky to auto-apply. The full review-card flow (pending-memory cards in the sidebar's change-review surface, alongside pending lore edits) is a follow-up tracked in `.planning/pr-memories.md`.
- **Kill switch**: `memoriesEnabled` off = no memory tools registered, no `MEMORY_DISCIPLINE_CLAUSE` in the prompt, no `buildMemoryMessage` injection. The feature vanishes entirely from the model's awareness — escape hatch for writers on small local models where every context token matters.
- **Prompt discipline**: the `MEMORY_DISCIPLINE_CLAUSE` (shipped via `buildInternalToolsMessage` when memories are on) tells the model WHEN to save (positive triggers: writer corrects an assumption, states a preference, surfaces a non-lore fact) and when NOT to (transient discussion, lore restatements, obvious manuscript facts). Scope guidance: `auto` default, `global` only for cross-manuscript facts.
- **UI**: "Memories" sub-tab under Lorebook (alongside Document / Manuscript / Relationships). Cards show heading + block ID + preview + tags. Click a card to open the underlying `.memories.md` at that section via Obsidian's `path#^id` link syntax. "New memory" button opens (creating if missing) the active scope's file. No plugin-level cache — re-reads on each render (memory files are small).
...
- **Single quotes, 120-col width, no trailing commas, semicolons required** — all **enforced by Prettier** (`prettier.config.mjs`), run by `build:dev`, `lint:fix`, `prettier:check`, and CI (via `lint:fix` only; the reproducible `build` does NOT run Prettier).
...
| Single quotes, 120-col, no trailing comma, semicolons                                                                       | Prettier via `build:dev` / `lint:fix` / `prettier:check` + CI | `prettier.config.mjs`                                                                                                                             |
...
| Obsidian-specific lint rules                                                                                                | `eslint.config.mts` + `eslint-plugin-obsidianmd` `^0.4.x` | All rules are `warn`-level in 0.4.x (mirror of the Obsidian plugin reviewer) except `obsidianmd/ui/sentence-case`, which we override back up to `error`. `prefer-setting-definitions` is active (2.2.0 declarative migration) — see below. |
...
- Example: `feature/2.2.0`, `feature/prose-linter`, `bugfix/settings-crash`
...
| `memoriesEnabled`                 | `save_memory`, `recall_memory`, `delete_memory` (memory CRUD — see "Memories")                                                                                                                                                | `Memories`                     | `Memories`                |
...
**Not every tool-related toggle belongs in this table.** Single-feature toggles like `reviewSuggestedEditsEnabled` (v2.2.0 — gates the proactive editor chat as a whole, not specific tools) and `enableFeedbackQueue` live in ONE Setting UI (no triplication) because they don't gate individual tools — they gate whether a feature surface appears. The triplication rule applies only when a single setting controls which tools register.

## Key feature areas

1. **Manuscript Context Engine** — auto-builds working context from open document.
2. **Manuscript Dashboard** — chapter word counts, pacing analysis, dialogue ratios, readability, and a deterministic narrative-flow score (0-100 composite of sentence-length variety, paragraph-length rhythm, pacing-flag density, and dialogue balance). 2.2.0 adds **Writing goals & sessions** (`src/core/dashboard/writing-goals.ts`): a daily words-written goal (default 500), a writing streak derived from a per-date ledger of manuscript word-count deltas, a best-ever streak, and a focus-session timer with idle-timeout auto-stop (default 20 min of inactivity; the credited duration excludes the idle tail) — all deterministic (no AI), persisted to `<pluginDataDir>/writing-goals.json`.
...
16. **Proactive Editor Chat (v2.2.0)** — after the initial report, the follow-up discussion in the Review tab runs through the co-writer session with editing tools enabled, so the editor can propose specific, reviewable inline-diff edits to any vault note — not just advisory prose. Every proposed edit still requires explicit writer approval before it reaches the vault. Gated by `reviewSuggestedEditsEnabled` (default on). See `'review-discuss'` mode under Tool-calling architecture and Snapshot-on-tab-switch under Conversation persistence.
17. **Memories** — cross-session AI-managed context, stored as writer-editable markdown in `Memories/`. The model saves durable facts it learns (preferences, intent behind flagged choices, worldbuilding notes that don't fit the typed-lore model) via `save_memory`; future sessions see them via an auto-injected index (`buildMemoryMessage`). Per-manuscript + global scope tiers, `^quill-mem-NNN` Obsidian block IDs as stable handles, hybrid retrieval (index by default, full bodies via `memoriesFullInject` or `recall_memory`). The writer edits memories directly in the markdown; the UI is a view (Lorebook "Memories" sub-tab). Gated by `memoriesEnabled` (default on); kill switch for small local models. See "Memories" architecture section.
18. **AI response language** (issue #57) — `aiResponseLanguage` (free-form, default empty) pins every user-facing AI RESPONSE surface (reviews, co-writer chat, linter explanations, transformations, subagent reports, continuation-option labels) to the writer's chosen language via `appendLanguageDirective` (`src/ai/prompts.ts`), appended to each surface's system prompt at construction. The directive deliberately keeps drafted/rewritten STORY PROSE tied to the manuscript's own language. Empty = no directive (model decides). Quill's English-authored system prompts otherwise dominate the model's language choice even when the provider-level system prompt or a custom instruction asks for another language — most OpenAI-compatible servers REPLACE the system prompt rather than merging.
...
- **Major** (x.0.0): For any event requiring a `minAppVersion` bump in `manifest.json` (e.g., adopting a new Obsidian API that drops older versions). 2.0.0 was the first major bump — it raised `minAppVersion` 1.7.2 → 1.13.0 to adopt the declarative settings API. Future major bumps follow the same rule.
