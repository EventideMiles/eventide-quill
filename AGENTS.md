...
- **Linting: ESLint** with `eslint-plugin-obsidianmd` `^0.4.x` (configured in `eslint.config.mts`). The 0.4.x line is the "community scanners ruleset" — it mirrors what the Obsidian plugin reviewer flags, and downgrades every rule to `warn` by default (none of them fail `npm run lint` on their own). We override `obsidianmd/ui/sentence-case` back up to `error` so a sentence-case violation will fail the build; everything else (including `prefer-create-el`) stays at `warn`, so `npm run lint` only exits non-zero on actual errors or stylelint/jscpd failures. The 0.4.x line also adds `prefer-create-el`, `prefer-setting-definitions`, `no-deprecated-display`, `no-nodejs-modules` to the recommended config. `prefer-setting-definitions` is active — 2.0.1 adopted the declarative `getSettingDefinitions()` API (see "Coding rules" below). Additionally, `eslint-plugin-jsdoc` `^63.3.2` provides the **`jsdoc/require-jsdoc` docstring gate** (also `warn`-level, so `npm run lint` stays green) — see "Coding rules" below.
...
src/
  ...
  core/
    ...
    dashboard/            # Manuscript dashboard + lorebook
      ...
      writing-goals.ts (2.0.1 daily words-written ledger + streak + focus-session core; pure logic + sidecar persistence),
...
### `'review-discuss'` mode (v2.0.1 — proactive editor chat)
...
| Obsidian-specific lint rules                                                                                                | `eslint.config.mts` + `eslint-plugin-obsidianmd` `^0.4.x` | All rules are `warn`-level in 0.4.x (mirror of the Obsidian plugin reviewer) except `obsidianmd/ui/sentence-case`, which we override back up to `error`. `prefer-setting-definitions` is active (2.0.1 declarative migration) — see below. |
...
16. **Proactive Editor Chat (v2.0.1)** — after the initial report, the follow-up discussion in the Review tab runs through the co-writer session with editing tools enabled, so the editor can propose specific, reviewable inline-diff edits to any vault note — not just advisory prose. Every proposed edit still requires explicit writer approval before it reaches the vault. Gated by `reviewSuggestedEditsEnabled` (default on). See `'review-discuss'` mode under Tool-calling architecture and Snapshot-on-tab-switch under Conversation persistence.
