...
- **Linting: ESLint** with `eslint-plugin-obsidianmd` `^0.4.x` (configured in `eslint.config.mts`). The 0.4.x line is the "community scanners ruleset" — it mirrors what the Obsidian plugin reviewer flags, and downgrades every rule to `warn` by default (none of them fail `npm run lint` on their own). We override `obsidianmd/ui/sentence-case` back up to `error` so a sentence-case violation will fail the build; everything else (including `prefer-create-el`) stays at `warn`, so `npm run lint` only exits non-zero on actual errors or stylelint/jscpd failures. The 0.4.x line also adds `prefer-create-el`, `prefer-setting-definitions`, `no-deprecated-display`, `no-nodejs-modules` to the recommended config. `prefer-setting-definitions` is active — 2.0.0 adopted the declarative `getSettingDefinitions()` API (see "Coding rules" below).
