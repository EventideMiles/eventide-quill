...
- **Toolset** = `createToolRegistry(plugin, false, false)` — same as `discuss`: all 14 internal tools (the base set plus `delete_paragraph` and `add_world_rule`), no `propose_entry`/`attach_lore_image`, no subagent spawners. Network/image tools follow their existing toggles. Locked down by `tests/ai/tools/tool-registry.test.ts`.
...
**Persistence limitation (v1.4.0):** background snapshots are in-memory only — `onunload` snapshots just the foreground (active) chat, matching the pre-1.4.0 contract. Writers who care about preserving a background chat should switch to that tab before closing Obsidian. Per-slot sidecar persistence is a v1.4.x follow-on.
...
16. **Proactive Editor Chat (v1.4.0)** — after the initial report, the follow-up discussion in the Review tab runs through the co-writer session with editing tools enabled, so the editor can propose specific, reviewable inline-diff edits to any vault note — not just advisory prose. Every proposed edit still requires explicit writer approval before it reaches the vault. Gated by `reviewSuggestedEditsEnabled` (default on). See `'review-discuss'` mode under Tool-calling architecture and Snapshot-on-tab-switch under Conversation persistence.
...
- **Major** (x.0.0): Only after the 1.4.0 release. For any event requiring a `minAppVersion` update in `manifest.json` (e.g., adopting a new Obsidian API that drops older versions). _Before 1.4.0, major bumps are NOT used — breaking changes use minor instead._
