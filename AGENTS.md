...
src/
  ...
  ai/                  # Provider architecture, streaming, prompts, tools, vision
    ...
    streaming.ts, transport.ts (HttpError, StreamingUnavailableError, the one `window.fetch` exception, looksLikeMobileNetworkDrop/withMobileNetworkHint),
...
- **Mobile-aware error framing** — `looksLikeMobileNetworkDrop` / `withMobileNetworkHint` (`src/ai/transport.ts`) recognize the common network-drop signatures ("Failed to fetch", `ERR_CONNECTION_*`, "websocket", etc.) and append a hint pointing at app backgrounding as the likely cause. Both providers (`openai-provider.ts`, `ollama-provider.ts`) wrap their mobile `requestUrl` call with `withMobileNetworkHint` so the raw exception string is replaced with an actionable message; desktop errors pass through unchanged.
