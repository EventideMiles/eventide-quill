...
**Deferred (post-1.3.0):** UI tests (`src/ui/*`, `main.ts`, `settings.ts`), CodeMirror decoration tests, and the heavy AI pipelines (`co-writer.ts`, `runFeedbackJob`). These need jsdom + lifecycle simulation — a separate, larger effort. (Provider integration tests are no longer deferred — see the `chatCompletion` suites below, driven by the `tests/helpers/mock-http.ts` window.fetch + requestUrl mock layer.)
...
- **Major** (x.0.0): Only after the 1.3.0 release. For any event requiring a `minAppVersion` update in `manifest.json` (e.g., adopting a new Obsidian API that drops older versions). _Before 1.3.0, major bumps are NOT used — breaking changes use minor instead._
