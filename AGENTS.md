...
- **Linting: ESLint** with `eslint-plugin-obsidianmd` `^0.4.x` (configured in `eslint.config.mts`). The 0.4.x line is the "community scanners ruleset" — it mirrors what the Obsidian plugin reviewer flags, and downgrades every rule to `warn` by default (none of them fail `npm run lint` on their own). We override `obsidianmd/ui/sentence-case` back up to `error` so a sentence-case violation will fail the build; everything else (including `prefer-create-el`) stays at `warn`, so `npm run lint` only exits non-zero on actual errors or stylelint/jscpd failures. The 0.4.x line also adds `prefer-create-el`, `prefer-setting-definitions`, `no-deprecated-display`, `no-nodejs-modules` to the recommended config. `prefer-setting-definitions` is disabled at the config level until Obsidian ships `getSettingDefinitions()` as a stable requirement — see "Coding rules" below.
...
5. `npm run typecheck:e2e` (typechecks the WDIO specs against WDIO/Mocha types — kept out of `build`'s typecheck because the WDIO globals would conflict with vitest), and
6. `npm run test:e2e` (mocked E2E — launches real Obsidian, ~8s wall-clock; hermetic via the in-process mock SSE/NDJSON server so no LM Studio dependency), and
7. `npm run test:e2e:live` (optional — exercises the real local model; auto-skips if LM Studio is unreachable), and
8. Manual smoke test in Obsidian (especially on mobile) for UI or provider changes that the mocked E2E suite doesn't exercise.
9. For release builds, also run `npm run build:release` to verify minification and `__DEV__` tree-shaking work correctly.

CI (`.github/workflows/lint.yml`) runs `build` + `test` + `lint` + `lint:dup` on every push and PR across Node 20/22/24. CI for releases (`.github/workflows/release.yml`) runs `build:release` instead. The mocked E2E suite runs in its own job (`.github/workflows/e2e.yml`) on every PR — Ubuntu-latest, latest Obsidian, single instance — adds ~5min to the PR check (the ~100MB Obsidian bundle is cached via `actions/cache`). The live suite is local-only and never in CI.
...
### `'review-discuss'` mode (v1.5.0 — proactive editor chat)
...
| Obsidian-specific lint rules                                                                                                | `eslint.config.mts` + `eslint-plugin-obsidianmd` `^0.4.x` | All rules are `warn`-level in 0.4.x (mirror of the Obsidian plugin reviewer) except `obsidianmd/ui/sentence-case`, which we override back up to `error`. `prefer-setting-definitions` is disabled at the config level — see below. |
...
### `getSettingDefinitions()` deferred until Obsidian ships it as a stable requirement

The 0.4.x obsidianmd plugin added `obsidianmd/settings-tab/prefer-setting-definitions`, which encourages `PluginSettingTab` subclasses to implement the declarative settings API (`getSettingDefinitions()`) so settings appear in Obsidian 1.13+ settings search. **We have this rule disabled at the config level** (`eslint.config.mts`) and will NOT implement `getSettingDefinitions()` until:

1. Obsidian ships `getSettingDefinitions()` as a **stable, required** API (not just an opt-in 1.13+ enhancement), AND
2. Our `minAppVersion` has naturally advanced to the point where the API is universally available to our installed users.

Adopting `getSettingDefinitions()` is itself the trigger for a **major version bump** (per "Version management" above), because raising `minAppVersion` to 1.13+ breaks compatibility with every currently released Obsidian build and forces existing users to upgrade. Until then, our `minAppVersion` stays at 1.7.2, `display()` continues to render the settings UI, and the deferred warning is silenced by config — not by per-site disables (the `eslint-comments/no-restricted-disable` rule blocks those anyway).
...
16. **Proactive Editor Chat (v1.5.0)** — after the initial report, the follow-up discussion in the Review tab runs through the co-writer session with editing tools enabled, so the editor can propose specific, reviewable inline-diff edits to any vault note — not just advisory prose. Every proposed edit still requires explicit writer approval before it reaches the vault. Gated by `reviewSuggestedEditsEnabled` (default on). See `'review-discuss'` mode under Tool-calling architecture and Snapshot-on-tab-switch under Conversation persistence.
...
- **Major** (x.0.0): Only after the 1.5.0 release. For any event requiring a `minAppVersion` update in `manifest.json` (e.g., adopting a new Obsidian API that drops older versions). _Before 1.5.0, major bumps are NOT used — breaking changes use minor instead._
