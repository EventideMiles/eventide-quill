...
- **Linting: ESLint** with `eslint-plugin-obsidianmd` `^0.4.x` (configured in `eslint.config.mts`). The 0.4.x line is the "community scanners ruleset" — it mirrors what the Obsidian plugin reviewer flags, downgrades every rule to `warn` (none of them fail `npm run lint` on their own), and adds `prefer-create-el`, `prefer-setting-definitions`, `no-deprecated-display`, `no-nodejs-modules` to the recommended config. `prefer-setting-definitions` is disabled at the config level until Obsidian ships `getSettingDefinitions()` as a stable requirement — see "Coding rules" below.
...
| Obsidian-specific lint rules                                                                                                | `eslint.config.mts` + `eslint-plugin-obsidianmd` `^0.4.x` | All rules are `warn`-level in 0.4.x (mirror of the Obsidian plugin reviewer). `prefer-setting-definitions` is disabled at the config level — see below. |
