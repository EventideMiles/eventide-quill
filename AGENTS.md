...
- **Package manager: npm** — pinned to `npm@11.16.0` via the `packageManager` field (corepack) and the CI pin (`npm install -g "npm@11.16.0"`). `engines.npm >= 11.16.0` is advisory only — `.npmrc` deliberately does **not** set `engine-strict=true`: doing so made `npm install` abort with `EBADENGINE` under older npm, which broke Obsidian's plugin submission checker (it installs with an npm older than 11.16.0, so the `obsidian` devDependency never landed and every Obsidian API read as `any` → thousands of spurious `@typescript-eslint/no-unsafe-*` warnings). The `allowScripts` fields in `package.json` are honored only by npm >= 11.16.0, which the `packageManager` field + CI pin guarantee for the project's own installs.
