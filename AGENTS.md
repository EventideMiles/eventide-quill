...
- **Linting: ESLint** with `eslint-plugin-obsidianmd` `^0.4.x` (configured in `eslint.config.mts`). The 0.4.x line is the "community scanners ruleset" — it mirrors what the Obsidian plugin reviewer flags, and downgrades every rule to `warn` by default (none of them fail `npm run lint` on their own). We override `obsidianmd/ui/sentence-case` back up to `error` so a sentence-case violation will fail the build; everything else (including `prefer-create-el`) stays at `warn`, so `npm run lint` only exits non-zero on actual errors or stylelint/jscpd failures. The 0.4.x line also adds `prefer-create-el`, `prefer-setting-definitions`, `no-deprecated-display`, `no-nodejs-modules` to the recommended config. `prefer-setting-definitions` is active — 2.1.0 adopted the declarative `getSettingDefinitions()` API (see "Coding rules" below). Additionally, `eslint-plugin-jsdoc` `^63.3.2` provides the **`jsdoc/require-jsdoc` docstring gate** (also `warn`-level, so `npm run lint` stays green) — see "Coding rules" below.
...
- **Duplication: jscpd v5** (Rust engine; dev-only, never bundled into `main.js`) — copy-paste detector gated in CI via `npm run lint:dup`. Configured in `.jscpd.json`: `minLines` 6, `minTokens` 70, `mode: weak` (skip comment tokens so JSDoc doesn't inflate the count), `threshold: 1.8` over duplicated **lines** (baseline 1.60% post-2.1.0 co-writer/main turn-extraction; **ratchet down** as duplication is extracted — the number should only ever decrease). The v5 Rust engine dropped v4's inline `jscpd:ignore-file` directive, so file-level exclusion lives in `.jscpd.json`, not at the code site.
...
| `live/lore-consistency.e2e.ts` (live)   | Real LM Studio run of the 2.1.0 lore-consistency critical mode (line-referenced report); auto-skips if unreachable |
...
src/
  ...
  core/
    ...
    dashboard/            # Manuscript dashboard + lorebook
      ...
      writing-goals.ts (2.1.0 daily words-written ledger + streak + focus-session core; pure logic + sidecar persistence),
...
### `'review-discuss'` mode (v2.1.0 — proactive editor chat)
...
| Obsidian-specific lint rules                                                                                                | `eslint.config.mts` + `eslint-plugin-obsidianmd` `^0.4.x` | All rules are `warn`-level in 0.4.x (mirror of the Obsidian plugin reviewer) except `obsidianmd/ui/sentence-case`, which we override back up to `error`. `prefer-setting-definitions` is active (2.1.0 declarative migration) — see below. |
...
16. **Proactive Editor Chat (v2.1.0)** — after the initial report, the follow-up discussion in the Review tab runs through the co-writer session with editing tools enabled, so the editor can propose specific, reviewable inline-diff edits to any vault note — not just advisory prose. Every proposed edit still requires explicit writer approval before it reaches the vault. Gated by `reviewSuggestedEditsEnabled` (default on). See `'review-discuss'` mode under Tool-calling architecture and Snapshot-on-tab-switch under Conversation persistence.
