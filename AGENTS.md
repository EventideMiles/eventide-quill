...
- **Linting: ESLint** with `eslint-plugin-obsidianmd` `^0.4.x` (configured in `eslint.config.mts`). The 0.4.x line is the "community scanners ruleset" — it mirrors what the Obsidian plugin reviewer flags, and downgrades every rule to `warn` by default (none of them fail `npm run lint` on their own). We override `obsidianmd/ui/sentence-case` back up to `error` so a sentence-case violation will fail the build; everything else (including `prefer-create-el`) stays at `warn`, so `npm run lint` only exits non-zero on actual errors or stylelint/jscpd failures. The 0.4.x line also adds `prefer-create-el`, `prefer-setting-definitions`, `no-deprecated-display`, `no-nodejs-modules` to the recommended config. `prefer-setting-definitions` is active — 2.0.1 adopted the declarative `getSettingDefinitions()` API (see "Coding rules" below). Additionally, `eslint-plugin-jsdoc` `^0.63.x` provides the **`jsdoc/require-jsdoc` docstring gate** (also `warn`-level, so `npm run lint` stays green) — see "Coding rules" below.
...
- **Major** (x.0.0): For any event requiring a `minAppVersion` bump in `manifest.json` (e.g., adopting a new Obsidian API that drops older versions). 2.0.1 is the first major bump — it raised `minAppVersion` 1.7.2 → 1.13.0 to adopt the declarative settings API. Future major bumps follow the same rule.
