...
describe('countWords', () => {
    it.each<{ input: string; expected: number }>([
        ...
        { input: 'line one\nline two\nline three', expected: 6 }
    ])('counts $expected words for "$input"', ({ input, expected }) => {
...
describe('countSentences', () => {
    it.each<{ input: string; expected: number }>([
        ...
        { input: 'Just a fragment', expected: 1 },
        { input: 'What? Stop! Now.', expected: 3 }
    ])('counts $expected sentences for "$input"', ({ input, expected }) => {
        expect(countSentences(input)).toBe(expected);
    });
});
