...
export interface EventideQuillSettings {
     ...
     * (the pre-2.1.1 behavior). See `src/ai/context-refinement.ts`.
     */
     ...
     * create (the pre-2.1.1 behavior) — escape hatch.
     */
     ...
     * prose). Off preserves the pre-2.1.1 text-only chat behavior. Default:
     * on.
     */
...
export class EventideQuillSettingTab extends PluginSettingTab {
    ...
    private renderBridgeContent(containerEl: HTMLElement, render: (content: HTMLElement) => void): void {
        ...
        // matching the pre-2.1.1 grouped look. Each tab renders into a single
     ...
     * pre-2.1.1 `this.refreshBridge()` full re-render. Falls back to `update()` when
    ...
    /** Restore-defaults action for the General page (resets across all tabs, matching pre-2.1.1 behavior). */
            ...
            },
            {
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        desc: 'After a report finishes, the follow-up discussion runs through the co-writer session with editing tools enabled, so the editor can propose specific, reviewable inline-diff edits (not just advisory prose). Every proposed edit still requires your approval before it reaches the vault. Turn off to keep the pre-2.1.1 text-only chat behavior. Default: on.',
