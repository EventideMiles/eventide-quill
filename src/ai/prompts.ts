...
export function getLoreCoachSystemPrompt(): string {
    return [
        ...
        '   the topic already has a note. If it does, use `edit_note` or',
        '   `append_to_note` to extend it rather than creating a duplicate.',
        ...
        '## Proposing a draft',
        '',
        'Use `edit_note` or `append_to_note` for changes to existing entries.',
        'Prefer `edit_note` in INSERT mode (pass `anchor`, not `old_text`) whenever you',
        'are ADDING a section or detail — it splices the new text in without replacing',
        'anything, so it cannot accidentally clobber surrounding content. Use REPLACE',
        'mode (old_text) only when you are actually changing existing wording.',
