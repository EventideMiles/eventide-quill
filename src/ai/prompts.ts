...
export function getReviewDiscussSystemPrompt(
    ...
    if (!engine || engine === 'generic') {
        return [
            ...
            '- Reserve `insert_note` for adding brand-new content. It cannot',
            '  remove existing text, so using it for rewrites creates duplicates.',
            '- Match the full passage being replaced so the review card shows a',
            '  clean before-and-after diff.',
            '- Break large rewrites into paragraph-by-paragraph edits. If you',
            '  need to rework a whole scene or section, call edit_note once per',
            '  paragraph rather than matching the entire section in one call.',
            '  Smaller edits are more reliable (old_text matches are easier to',
            '  get right) and give the writer cleaner, more granular review cards.',
            '',
            ...
            '(the tool_calls field). A tool invocation written as text like',
            '`edit_note(...)` in your response does not execute \u2014 the writer',
            'sees raw syntax and nothing happens. If a previous turn failed to',
            'invoke a tool this way, the system may nudge you to re-issue it',
            'correctly \u2014 take the nudge.',
            '',
            '## Scope',
            '',
    ...
    return [
        ...
        '## Tool discipline (critical)',
        '',
        'Always invoke tools through the structured tool-calling interface (the',
        'tool_calls field). A tool invocation written as text like',
        '`edit_note(...)` or `vault_lookup(...)` in your response does not',
